...
Sym <- unlist(read.table(file = GeneList, sep = "\t", stringsAsFactors = FALSE, header = FALSE))
...
p <- ggplot(kk, aes(x=-log(pvalue), y=Description, color=-log(pvalue))) + 
...
#ego_up <- enrichGO(gene = Up, keyType = "SYMBOL", universe = all_genes$SYMBOL, OrgDb = org.Hs.eg.db, ont = "BP", pAdjustMethod = "BH", pvalueCutoff  = 0.1, qvalueCutoff  = 1, readable = FALSE)
...
write.table(gg, file = paste0(GeneList, "_GO_enrich.xls"), sep = "\t", row.names = FALSE, quote = FALSE)
...
p <- ggplot(gg, aes(x=-log(pvalue), y=Description, color=-log(pvalue))) +
