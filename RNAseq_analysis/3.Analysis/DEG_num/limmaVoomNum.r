...
## TMP as input and will be log transformed
...
#Factor <- Factor[!is.na(Factor[[Focus]]), ]
...
ggsave(paste0(Focus, "_MDSPlot.pdf"))
...
if(type=="paired"){
	...
	Ann <- read.delim(file = "/sc/arion/projects/zhangw09a/PANDA/db_ZS/Ensembl/Ann_ensembl_refseq_gene.xls", stringsAsFactors = F)
	...
	if(AdjustBy != "No"){
		...
		design <- model.matrix(as.formula(paste0(" ~ ", Focus, " + ", paste(AdjustBy, collapse = " + "))), Factor)
	...
	#for(i in 1:length(Contrast)){
		...
		result <- topTable(fit, coef=Focus, adjust="BH", number = 50000)
		...
		if(IDType == "Symbol"){
	        	...
	        	result.ann <- merge(result, Ann, by.x = 0, by.y = 1)
			...
			result.ann <- merge(result, Ann, by.x = 0, by.y = 2)
	        ...
	        result.ann <- merge(result.ann, Expr, by.x = 1, by.y = 0)
       		...
       		write.table(result.ann, file = paste("DEG", "diff.ann.xls", sep="_"), sep = "\t", quote = F, row.names = FALSE)
