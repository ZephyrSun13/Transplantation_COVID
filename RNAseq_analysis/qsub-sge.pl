...
my ($Queue, $Interval, $Lines, $Maxjob, $Convert,$Secure,$Reqsub,$Resource,$node,$Time,$Job_prefix,$Verbose, $Help,$Pro_code);
GetOptions(
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
	"interval:i"=>\$Interval,
...
while (@Shell) {
	...
	my $job_cmd = "bsub -L /bin/bash -n 1 -R span[ptile=1] ";  ## -l h_vmem=16G,s_core=8 
	$job_cmd .= "-q $Queue "  if(defined $Queue); ##set queue
	$job_cmd .= "-P $Pro_code " if (defined $Pro_code);##set -P,added by luchx
    	$job_cmd .= "-R rusage[mem=$Resource] " if(defined $Resource); ##set resource
	$job_cmd .= "-W $Time " if(defined $Time);
	...
	for (my $i=0; $i<@Shell; $i++) {
		while (1) {
			...
			if ($i < $Maxjob || ($run_num != -1 && $run_num < $Maxjob) ) {
				my $jod_return = `$job_cmd -o $Shell[$i].stdout -eo $Shell[$i].stderr -J $Shell[$i] <$Shell[$i]`;
				sleep 5; ## give the server some time to show the tasks
				my $job_id = $1 if($jod_return =~ /Job <(\d+)>/);
	...
	foreach my $job_id (sort keys %Alljob) {
		...
		##read the .o file
		my $content;
		if (-f "$shell_file.stdout") {
			open IN,"$shell_file.stdout" || warn "fail $shell_file.stdout";
			$content = join("",<IN>);
			close IN;
		}
		...
		if ($content !~ /This-Work-is-Completed!/) {
			...
			print OUT "In qsub cycle $qsub_cycle, In $shell_file.stdout,  \"This-Work-is-Completed!\" is not found, so this work may be unfinished\n";
		}
		

		##read the .e file
		my $content;
		if (-f "$shell_file.stderr") {
			open IN,"$shell_file.stderr" || warn "fail $shell_file.stderr";
			$content = join("",<IN>);
			close IN;
		}
...
sub run_count {
	...
	foreach my $job_line (@jobs) {
		...
		#print (join "\t", @job_field);
		#print "\n";
		next if($job_field[1] ne $user);
		#print "$job_field[0]\n";
		#print "Hash:";
		#print "$_\t" for keys $all_p;
		#print "\n";
		if (exists $all_p->{$job_field[0]}){
			#print "in\n";
			my %died;
			died_nodes(\%died); ##the compute node is down, 有的时候节点已死，但仍然是正常状态
			print STDERR join("\n", keys(%died)) if($Verbose);
			my $node_name = $1 if($job_field[6] =~ /(node\d+-\d+)/);
			#print $node_name."\n";
			#print $job_field[3]."\n";
			if ( !exists $died{$node_name} && ($job_field[3] eq "PEND" || $job_field[3] eq "RUN") ) {  
				$run_p->{$job_field[0]} = $job_field[2]; ##job id => shell file name
				$run_num++;
			}else{

				print STDERR "bkill $job_field[0]\n" if($Verbose);
				`bkill $job_field[0]`;
			}
		}
	}
...
sub died_nodes{
	...
	foreach  (@lines) {
		...
		my $node_name = $t[0];
		my $memory_use = $t[5];
		$died_p->{$node_name} = 1 if($t[1] eq "unavail");
	}
